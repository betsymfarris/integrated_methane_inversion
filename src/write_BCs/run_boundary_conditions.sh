...
condaEnv=$(grep -Po 'condaEnv:\s*\K.*' config_boundary_conditions_bf.yml)
condaFile=$(grep -Po 'condaFile:\s*\K.*' config_boundary_conditions_bf.yml)
condaFile=$(eval echo "$condaFile")
source ${condaFile}
conda activate ${condaEnv}
eval $(python ../../src/utilities/parse_yaml.py config_boundary_conditions_bf.yml)
source ${geosChemEnv}
...
# Commenting out new directory to maintain slurm changes (CURC)
if [[ -d "${workDir}" ]]; then
...
cp /projects/befa4441/integrated_methane_inversion/src/write_BCs/geoschem.run .
#sed -i -e "s|sapphire,huce_cascade,seas_compute,shared|${partition}|g" \
#    -e "s|--mem=15000|--mem=64000|g" \
#    -e "s|-t 0-12:00|-t 0-23:00|g" \
#    -e "s|--ntasks=8|--ntasks=8|g" \
#    -e "s|-c 8|-c 1|g" \
